...
RWIDTH=$(echo $(echo "$LIST" | head -n 1 | awk '{print length($0); }' |bc) +10|bc) 
...
if [[ ! -z $CURRSSID ]]; then
	  HIGHLINE=$(echo  "$(echo "$LIST" | awk -F "[  ]{2,}" '{print $1}' | rg -n -m 1 "$CURRSSID" | awk -F ":" '{print $1}') + 1" | bc ) 
fi

echo "gggfgg  $HIGHLINE"
...
FIELDSARR=$(cat $CONFIG | awk 'BEGIN { FS=","; OFS="\n" } /^FIELDS/ { $1 = substr($1, 8); print $0; }')
